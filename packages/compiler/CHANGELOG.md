...
- 9ad8598: - Refuse a `css.create`, `css.createTheme` or `css.createStatic` call that is not assigned to a named top-level variable, instead of skipping the declaration and emitting nothing for it. A default-exported or destructured call used to leave its styles out of the sheet with no diagnostic
...
- dea2ce8: - Fix a style handed to a component through a prop named the same as the one elements take. It was resolved to class names at the call site and the attribute was renamed, so the component never received it and the style was dropped with no error. This is the spelling the `StyleProps` reference shows
...
- 0b56e62: Fix: change x.index and entry.index to x.key and entry.key in compiler
...
- c562899: refactor: impact scanAll argument
...
- 7e4cff1: perf: deep nesting support and optimized
...
- 3fb4cd5: fix: compilation speed optimize
...
- 1061552: chore: build func is changed to Queue object
...
- aa00efa: chore: version bump only

## 0.22.2

### Patch Changes

- e9cc299: chore: version bump only
