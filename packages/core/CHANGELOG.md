...
- ddc0c29: - Fix: `@plumeria/core` to expose `overflowInline` instead of `overflowBlockX`. CSS has no `overflow-block-x` property; the logical counterpart of `overflow-x` is `overflow-inline`, and the type was reachable under a name that does not exist.
