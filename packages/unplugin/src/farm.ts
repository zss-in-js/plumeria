...
/**
 * Disk-based HMR for Farm.
 *
 * Farm's virtual module HMR has limitations:
 *   - Returning .zero.css paths from updateModules causes Rust core panic
 *     ("ModuleMetaData is not Script but custom")
 *   - invalidateModule doesn't push CSS reload signals to the browser
 *
 * Solution (same as turbopack-loader):
 *   - Dev: write CSS to a real file on disk, import it as a regular CSS file.
 *     Farm's file watcher detects changes and triggers HMR automatically.
 *   - Build: use the standard virtual CSS module pipeline (resolveId → load)
 *     so CSS is extracted to static files by Farm's production bundler.
 */
...
function attachFarmHooks(plugin: any) {
  ...
  plugin.transform = async function (this: any, code: string, id: string) {
    ...
    if (isDev && result && typeof result === 'object' && result.code) {
      ...
      // Replace virtual CSS imports with a reference to the real disk file
