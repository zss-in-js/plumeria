// Regression harness for the watch set the plugin registers per file.
//
...
//
// Vite watches the transformed module itself, so unlike the loader the plugin
// never registers the file it is compiling.
//
...
describe('unplugin: watch set', () => {
  it('watches the parents that pass it a style prop', async () => {
    expect(await watchedBy(LEAF)).toEqual([PARENT]);
  });

  it('watches the modules it imports, transitively', async () => {
    expect(await watchedBy(CARD)).toEqual([STYLES, TOKENS].sort());
  });

  it('watches both directions at once', async () => {
    expect(await watchedBy(PARENT)).toEqual([STYLES, TOKENS, LEAF].sort());
  });

  it('watches nothing for a file that neither imports nor receives styles', async () => {
    expect(await watchedBy(UNRELATED)).toEqual([]);
  });
