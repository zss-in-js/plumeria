...
export function scanAll(): Tables {
  ...
  // Third phase: every `css.create` is registered by now, so style variables
  // imported from another file resolve the same way no matter what order the
  // files came back from the glob in.
  for (const entry of jsxPhaseQueue) {
    try {
      ...
      // Imports were mapped to hashes during pass 2, while createHashTable was
      // still filling up. Re-resolve them against the completed table.
