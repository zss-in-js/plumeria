...
describe('turbopack-loader: bracket access inside conditionals', () => {
  it('keeps both branches of a ternary', async () => {
    const out = await run(
      `export const A = ({ on }: { on: boolean }) => <div styleName={on ? s['p1'] : s['p3']} />;`,
    );
  ...
  it('keeps the right-hand side of a logical &&', async () => {
    const out = await run(
      `export const A = ({ on }: { on: boolean }) => <div styleName={on && s['p1']} />;`,
    );
    expect(out).toContain('x3git8yv');
  });
  ...
  it('keeps a bracket branch nested in an array', async () => {
    const out = await run(
      `export const A = ({ on }: { on: boolean }) => <div styleName={[s.p3, on && s['p1']]} />;`,
    );
    expect(out).toContain('x3git8yv');
    expect(out).toContain('xxcejlqg');
  });
  ...
  it.each([
    ...
    await expect(
      run(
        `export const A = ({ on, k }: { on: boolean; k: 'p1' | 'p3' }) => <div styleName={${expr}} />;`,
      ),
  ...
  it('compiles the same intent with the condition inside the brackets', async () => {
    const out = await run(
      `export const A = ({ on, k }: { on: boolean; k: 'p1' | 'p3' }) => <div styleName={s[on ? k : 'p3']} />;`,
    );
    expect(out).toContain('x3git8yv');
    expect(out).toContain('xxcejlqg');
  });
  ...
  it('still compiles a non-literal key outside any condition', async () => {
    const out = await run(
      `export const A = ({ k }: { k: 'p1' | 'p3' }) => <div styleName={s[k]} />;`,
    );
    expect(out).toContain('x3git8yv');
    expect(out).toContain('xxcejlqg');
  });
