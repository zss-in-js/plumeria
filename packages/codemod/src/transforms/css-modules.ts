...
/** `font-size` -> `fontSize`, `-webkit-line-clamp` -> `WebkitLineClamp` */
...
/** The at-rules a rule can sit inside, innermost last. */
...
interface Target {
  /** the class the declarations land on */
  key: string;
  /** pseudo-classes written on that class, e.g. `:hover` */
  pseudo: string;
  /** set when the selector was a descendant or child pair */
  ancestor?: { key: string; pseudo: string };
}

/**
 * Reads one selector. Anything outside "a local class, optionally under another
 * local class" is left for the caller to report — an unrecognised shape never
 * falls through to a best effort.
 */
const readSelector = (
  selector: string,
