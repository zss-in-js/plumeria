...
`AtomicClassStyleFor` is the type of one entry of a dynamic style, which is what calling a [function key](/docs/api-reference/javascript/create#dynamic-values) gives. It reads the same as [AtomicClassNameFor](/docs/api-reference/types/AtomicClassNameFor) — a `string` that carries the property it was generated for — and adds where the value has to go.

A static atomic class name is a class name and nothing more, so it can be handed to `className`. A dynamic style reaches the element through `classStyle`, because the values the caller computed ride along as custom properties, and a class name has no room for them. The two types say which is which.
...
Calling a function key gives the map of what it produced, one entry per property:
