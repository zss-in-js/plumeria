...
An entry of a dynamic style is [AtomicClassStyleFor](/docs/api-reference/types/AtomicClassStyleFor), which is this type with the mark that says it reaches the element through `classStyle`. It satisfies `AtomicClassNameFor`, so a prop written this way takes either kind:
