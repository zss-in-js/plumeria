...
A dynamic style satisfies `AtomicStyle` too, as long as it sets the properties named:
...
When only a dynamic style will do, [AtomicClassStyle](/docs/api-reference/types/AtomicClassStyle) says so. When it must not be one, [StaticStyles](/docs/api-reference/types/StaticStyles) does.
