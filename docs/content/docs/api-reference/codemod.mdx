...
An import that only existed to feed the styles is removed once nothing else in the file reads it. A binding still used outside the exported styles — `window.matchMedia(breakpoints.lg)`, for instance — keeps its import.
...
+   <a className={`${styles.base} ${styles['hoverStyles-base']}`} />
  );
```
...
### Cascade after the export

Plumeria merges composed styles by property name at build time and emits one declaration per property. The export writes one class per `css.create` key instead, so the cascade decides. Three cases change:

- **Composition order.** `classStyle={[styles.b, styles.a]}` resolves `color` to `styles.a`, because the rightmost style wins the merge. Exported, `.a` and `.b` both land on the element and the rule written later in the stylesheet wins — that is, the later `css.create` key.
- **A conditional against a base declaration.** `color` and `@media (…): color` are separate entries that never merge, so the conditional carries `:not(#\#)` and wins regardless of order. Exported, both are ordinary rules and source order decides.

Nothing is dropped — every declaration is exported — but a project relying on either should order the affected pairs itself.
