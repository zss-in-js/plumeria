...
`@plumeria/unplugin/factory` exposes the compiler behind every bundler entry as a plain function. It takes a source string and hands back the rewritten code, with the stylesheet available from `resolveId` and `load`:
...
No bundler, no DOM and no runtime resolution of `@plumeria/core` is involved, so it runs anywhere plain JavaScript does. The hooks and the `addWatchFile` the transform reads off its context are part of the published surface. See [Testing](/docs/testing).
