...
The rewritten code comes back through `async`. The stylesheet is written only under `NODE_ENV=development`, and those writes are taken under a lock, so parallel callers do not lose rules. Those four context properties are part of the published surface. See [Testing](/docs/testing).
