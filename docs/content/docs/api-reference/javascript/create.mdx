...
<Callout type="warn" title="Before 18.1">
  A condition placed around the brackets was rejected at build time, and had to be
  moved inside. Both forms compile now, so the workaround is no longer needed.
</Callout>
