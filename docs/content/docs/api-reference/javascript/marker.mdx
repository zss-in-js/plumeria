...
description: 'Style descendants from a parent state without writing a combinator.'
---
...
<Callout type="info">
  **Production ready.** These APIs compile to CSS container style queries (`@container style()`), which current versions of Chrome, Edge, Safari and Firefox all support.
</Callout>
    ...
    const MarkerExtended = () => {
      return (
        <div classStyle={styles.grand}>
          <span classStyle={styles.description}>Grand Container (Hover me)</span>
          <div classStyle={styles.parent}>
            <span classStyle={styles.description}>Parent Container (Hover me too)</span>
...
That last part is why [`migrate --from css-modules`](/docs/api-reference/tools/codemod)
