...
Plumeria treats code that cannot be statically parsed as non-existent. Unknown dynamic values passed, such as `styleName={props.color}`, are ignored. This is a contract with the 0-byte runtime to prevent unknown props from being passed around at the design level.
</Callout>