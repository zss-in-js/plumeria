...
On Next.js the two `pre` scripts clear the build cache, which is what keeps a version change from being read as a compile error:
...
A `predev` or `prebuild` the project already wrote is left as it is.

What counts as already set up is the plugin being *called*, not merely imported — an import nothing uses is completed rather than skipped, and the binding the file already named is the one that gets called. So a second run writes nothing, and a half-finished one is finished.
...
Without a TTY it never asks — every unanswered question takes its default, which is what makes it usable in CI.

## Installation

None. Run it with `npx` and it is gone again afterwards. Install it only if you want it pinned in the repo:
