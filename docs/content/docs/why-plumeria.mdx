...
Measured on identical Next.js apps, that is 1.83KB less client JavaScript than StyleX, which keeps its resolver — see the [side-by-side benchmark](https://github.com/refirst11/css-compiler-bench).
...
Plumeria is for those who say, "I never compromise." Ready to try it? Start with [Getting started](/docs/installation).
