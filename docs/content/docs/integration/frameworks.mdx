---
title: 'Frameworks'
description: 'Use Plumeria with Vue, Svelte, Astro, Qwik and other frameworks Vite can build.'
---
...
The transform parses TypeScript and TSX. A `.vue`, `.svelte` or `.astro` file is neither, so `css.create` goes in a `.ts` module the component imports from. The template is not JSX either, so the module also resolves the class name with `css.use` and exports the string:
...
export const mainClass = css.use(styles.main);
```

```html title="src/components/Card.vue"
<script setup lang="ts">
import { mainClass } from './style';
</script>

<template>
  <div :class="mainClass">card</div>
</template>
```
