---
title: 'Pseudo'
description: 'Every pseudo-class and pseudo-element you can use as a key, grouped by what it targets.'
---

## Overview

Every standard pseudo-class and pseudo-element works as a key in `css.create()`. The tables below group them by what they target. For how they nest with queries, see [Selector rules](/docs/selector-rules).
...
| `:state(name)` | Custom state for custom elements | Style custom element based on state |
...
| `:popover-open` | Popover element that is open | Style active popovers |
| `:host` | Shadow DOM host element | Style host component |
| `:host(selector)` | Shadow DOM host matching selector | Style host component conditionally |
| `:host-context(selector)` | Shadow DOM host based on ancestor | Contextual component styling |
...
| `::target-text` | Element text targeted by URL fragment | Highlight target text fragment |
| `::highlight(name)` | Custom highlight registered via API | Custom text highlighting |
...
| `::file-selector-button` | Button of a file input | Style file uploads |
...
| `::details-content` | Content container of details element | Style open accordion contents |
