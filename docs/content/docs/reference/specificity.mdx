...
width of 1000px both queries below apply, and the later rule wins. Written in this
order the narrower query wins, which is what you meant by writing it. Reversed,
the broader one wins, and `no-order-dependent-overlap` says so. Composed from
two modules, later means the order the atoms were emitted in: the order of the
`classStyle` array, unless one of the two is also used somewhere the compiler
reaches first, which pins it. The file list is sorted, so the result is the same
on every machine, but it is not always the order you composed them in.
...
`no-order-dependent-overlap` reports all three when they meet in one style, the
conditions only where one provably matches a subset of the other.
</Callout>
