---
title: 'Introduction'
description: 'What Plumeria is, what writing styles with it looks like, and how far its types reach.'
---
...
Because the compiler controls the merge, it settles specificity instead of the cascade, apart from the few pairs listed in [Specificity](/docs/specificity). Writing the merge as an operation, where $\oplus$ combines styles left to right, disjoint styles simply concatenate:
...
A shorthand against a longhand is the exception: the longhand wins wherever the two meet, in either order. The laws this merge obeys are in [Composition laws](/docs/composition-laws).
...
To learn what motivated this design, read [Why Plumeria?](/docs/why-plumeria). For integration with bundlers and the full API, see the [API reference](/docs/api-reference) and [Installation](/docs/installation).
