...
Get started with **Plumeria** using `css.create()` and `css.use()` to build styles with clarity and structure.
...
export function Box() {
  return (
    <div className={css.use(stylesBox.container)}>
      <span>
        <span tabIndex={0} className={css.use(stylesBox.card, animated.hover1)}>
          A
        </span>
      </span>
      <span>
        <span tabIndex={0} className={css.use(stylesBox.card, animated.hover2)}>
          B
        </span>
      </span>
    </div>
