...
styleName={styles.base, styles.custom}
styleName={styles.custom, styles.base}
...
<Step>
## `styleName` prop merge styles

Also, when `styleName` prop has duplicate properties on its arguments, the style properties on the right always take precedence and duplicate properties on the left are filtered. In that case no filtered hash is generated.
...
styleName={styles.base, styles.custom} // 16px
styleName={styles.custom, styles.base} // 10px
```
...
- Media query styles always take precedence over base styles.
- Minimal specificity adjustments in production to keep bundle sizes small.
</Step>
</Steps>