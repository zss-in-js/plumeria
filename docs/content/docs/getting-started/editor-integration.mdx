...
In VScode create a shortcut for the css.use() object expansion in className.  
`shift + command + p >`  
...
```json
{
  "React className": {
    "prefix": "css",
    "body": "className={css.use($1)}",
    "description": "React className with css.use()",
  }
}
```
...
### Macro Usage
```jsx
<div css /> -> Enter -> <div className={css.use(|)} />
```
</Step>
<Step>
## ESLint
