---
title: 'Recipes'
description: 'Learn more about Plumeria recipe'
---

import { FancyButtons, Box } from 'component/Recipes'

Get started with **Plumeria** using `style.create()` and `style.use()` to build styles with clarity and structure.
...
export function Box() {
  return (
    <div className={style.use(stylesBox.container)}>
      <span>
        <span tabIndex={0} className={style.use(stylesBox.card, animated.hover1)}>
          A
        </span>
      </span>
      <span>
        <span tabIndex={0} className={style.use(stylesBox.card, animated.hover2)}>
          B
        </span>
      </span>
    </div>
...
const scaleOut = style.keyframes({
  from: {
...
const rotatationOut = style.keyframes({
  from: { scale: 1 },
  to: {
    scale: 1.2,
    opacity: 0,
