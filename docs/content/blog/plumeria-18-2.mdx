...
Adopting a styling engine should not be a one-way door. The codemod runs in reverse just as mechanically:
...
Like the forward migration, start with `--dry-run` to inspect what would change. Anything with dynamic semantics that cannot be cleanly converted is reported and left untouched.
