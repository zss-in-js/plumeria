...
Plumeria merges composed styles by property name before it emits anything. `classStyle={[styles.a, styles.b]}` resolves `color` once — rightmost wins — and a single declaration reaches the element. A conditional is keyed separately: `color` and `@media (…):color` are two entries that never merge, so the conditional carries `:not(#\#)` and outranks the base declaration whatever order they land in.

The export writes one class per `css.create` key, so the cascade decides instead. Two composed styles setting the same property both reach the element, and the rule written later in the stylesheet wins rather than the one written further right in the array. A conditional against a base declaration falls back to source order for the same reason. Every declaration is exported; the order they were merged in is not, because a stylesheet has no syntax for it.
