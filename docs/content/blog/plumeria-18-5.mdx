...
The unplugin transform now carries every patch the Turbopack loader already had, so both bundlers read the same file the same way, and the retired `css.variants` path is gone in favor of the bracket notation on `css.create`.
