...
Get started with **Plumeria** using `css.create()` and `css.createComposite()` to build styles with clarity and structure.
...
### 2. Compose pseudo interactions
Extend your styles with pseudo interactions using `css.createComposite():`
...
export function FancyButtons() {
  return (
    <div className={base.stack}>
      ...
      <button className={buttons.focusSky}>Sky</button>
      <button className={buttons.focusGreen}>Green</button>
    </div>
  );
}
```
